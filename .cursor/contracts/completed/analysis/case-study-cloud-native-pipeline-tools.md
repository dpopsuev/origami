# Contract — Case Study: Cloud-Native Pipeline Tools

**Status:** draft  
**Goal:** Analyze five CNCF cloud-native tools against Origami's pipeline model, map integration points, identify competitive gaps, and extract actionable tasks to existing contracts.  
...
Cloud-native is an important pillar. Origami pipelines run in environments that already have Prometheus, Jaeger, Argo, and Kubernetes. Day-0 integration with this ecosystem is a competitive advantage over Python-based frameworks (CrewAI, LangGraph) that require custom observability wrappers.

### Tools to analyze

| Tool | CNCF Status | Category | Hypothesis |
|------|-------------|----------|------------|
| **Argo Workflows** | Graduated | Workflow orchestration | Origami `PipelineDef` as Argo Workflow template. Long-running production pipeline execution on K8s. Infrastructure integration. |
| **Tekton** | Graduated | CI/CD pipelines | Relevant for Asterisk's RP integration (analyzing Tekton-triggered failures). Potential Asterisk adapter. |
| **KEDA** | Graduated | Event-driven autoscaling | Scale pipeline workers based on queue depth. Pairs with `pipeline-efficiency` contract. Infrastructure integration. |
...
- `pipeline-efficiency` — KEDA autoscaling is related to pipeline worker scaling
- `origami-k8s-pipeline-operator` (completed design) — Argo analysis builds on the K8s operator design
...
|----------|--------|-------------|
| Cloud-native integration analysis | `docs/case-studies/cloud-native-pipeline-tools.md` | domain |
...
- [ ] **A1** Research Argo Workflows: architecture, Workflow CRD, template types, parameter passing. Map to Origami PipelineDef/Node/Edge.
- [ ] **A2** Research Tekton: Pipeline/Task/Step model, Tekton Results, integration with CI triggers. Map to Origami pipeline and Asterisk RP integration.
