# Contract — origami-dsl-pipeline-vars

**Status:** complete  
**Goal:** Pipelines declare variables (`vars:`), inter-node data flow (`input:`), and prompt templates (`prompt:`) in YAML. Thresholds, configuration, and data flow are fully declarative.  
**Serves:** Origami DSL
...
|----------|--------|-------------|
| Pipeline variables reference | `docs/` | domain |
| Data flow pattern documentation | `docs/` | domain |

## Execution strategy

Phase 1: Add `Vars` to `PipelineDef` and implement variable resolution. Phase 2: Add `Input` to `NodeDef` and implement inter-node data flow. Phase 3: Integrate prompt template rendering with unified context. Phase 4: Migrate Asterisk thresholds and params. Phase 5: Validate, tune, validate.
...
| **Integration** | yes | Full pipeline with vars flowing into expressions and transformers |
| **Contract** | yes | `vars:` and `input:` YAML fields accepted and resolved correctly |
| **E2E** | yes | Asterisk pipeline with `vars:` replacing `Thresholds` struct |
| **Concurrency** | no | Variables are immutable after pipeline load |
...
**Given** a pipeline YAML with `vars:` and `input:` declarations,  
...
2026-02-23 12:00 — Contract created. Depends on `origami-dsl-transformers`. This contract makes the pipeline YAML self-sufficient by declaring all configuration and data flow inline.
