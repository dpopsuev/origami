...
**Status:** complete  
**Goal:** `origami run circuit.yaml --input data.json` works as a standalone CLI. The framework is a complete circuit execution engine. Domain tools become thin wrappers.  
**Serves:** Origami DSL
...
- **The low floor.** This contract delivers the Papert entry point: write a YAML circuit, run `origami run`. No Go code needed for the common case.
...
- `origami-dsl-hooks` — Predecessor. Provides the complete circuit execution model: transformers + schemas + expressions + hooks.
...
- `origami-dsl-circuit-vars` — Provides `vars:`, `input:`, prompt templates.
...
| **Integration** | yes | `origami.Run()` executes full circuit with transformers + hooks + expressions |
| **Contract** | yes | CLI flag contract: `--input`, `--set`, positional circuit path |
| **E2E** | yes | Asterisk stub calibration via `origami.Run()` produces identical metrics |
| **Concurrency** | no | Single circuit execution per invocation |
...
**When** a user runs `origami run circuit.yaml --input data.json --set recall_hit=0.9`,  
**Then**:
- The circuit loads, expressions compile, schemas validate, graph builds
- The walk executes: transformers process nodes, schemas validate artifacts, expressions route edges, hooks fire side effects
- `origami validate circuit.yaml` catches errors without executing
...
|-------|---------|------------|
| A01 Path Traversal | `--input` and positional circuit path accept file paths from user. | Resolve paths relative to CWD. Reject absolute paths outside workspace. |
| A05 Misconfiguration | `--set` allows overriding any circuit variable. A typo could disable safety thresholds. | Log all overrides at startup. `origami validate` warns about unknown variable names. |

## Notes

2026-02-23 12:00 — Contract created. Capstone of the Origami DSL initiative. Depends on all 4 prior contracts. Delivers the Papert low floor: `origami run circuit.yaml`.
