...
**Status:** complete
**Goal:** Define a declarative YAML/Go DSL for expressing circuits as graphs, so that the F0-F6 circuit, the Defect Court D0-D4 circuit, or any future circuit can be declared as a configuration rather than hardcoded in Go.
...
- The DSL must map 1:1 to `framework.Graph` -- every DSL circuit produces a valid `Graph` instance.
- The F0-F6 circuit must be expressible in the DSL as the first validation case.
...
|-----------|------------------------------|
| P1: YAML as canonical | One `.yaml` file per circuit in `circuits/`. All other formats derived. |
| P2: Declarative intent | `condition` fields are human-readable labels; Go `NodeRegistry` holds executable logic. |
| P3: Reading-first layout | YAML sections ordered: circuit > zones > nodes > edges > start/done. Edges sorted by source node. |
...
| P6: Derived visualization | `Render(*CircuitDef) string` generates Mermaid from the parsed definition. |
| P7: Progressive disclosure | `zones`, `element`, `stickiness` are optional with sensible defaults. Minimal circuit needs only `nodes`, `edges`, `start`, `done`. |
| P8: Round-trip fidelity | `LoadCircuit -> CircuitDef -> MarshalYAML` produces semantically equivalent YAML. |
...
- `internal/orchestrate/types.go` -- `CircuitStep` enum that defines F0-F6 node identifiers.
...
description: "F0-F6 root cause analysis circuit"
...
// BuildGraph constructs a Graph from a CircuitDef and a NodeRegistry.
// The NodeRegistry maps node names to Node implementations.
func (def *CircuitDef) BuildGraph(registry NodeRegistry) (Graph, error)
...
1. Define `CircuitDef`, `ZoneDef`, `NodeDef`, `EdgeDef` structs with YAML tags. Apply progressive disclosure (P7): optional fields use `omitempty`, defaults are zero-values.
2. Implement `LoadCircuit` -- parse YAML into `CircuitDef`.
3. Implement `CircuitDef.Validate()` -- check referential integrity (all edge endpoints exist, zones reference valid nodes, start node exists).
4. Implement `BuildGraph` -- construct a `Graph` from the definition + a `NodeRegistry`.
5. Implement `Render(*CircuitDef) string` -- generate Mermaid flowchart from a parsed circuit definition (P6: derived visualization).
6. Express the F0-F6 circuit as a YAML file and verify round-trip (P8): YAML -> CircuitDef -> Graph -> walk.
7. Express the Defect Court D0-D4 circuit as a second YAML file for validation.
8. Verify round-trip fidelity (P8): `LoadCircuit -> Marshal -> LoadCircuit` produces equivalent results.

## Tasks

- [x] Create `internal/framework/dsl.go` -- `CircuitDef`, `ZoneDef`, `NodeDef`, `EdgeDef` structs (P7: optional fields with `omitempty`)
- [x] Implement `LoadCircuit(data []byte) (*CircuitDef, error)` -- YAML parser
- [x] Implement `CircuitDef.Validate() error` -- referential integrity checks
...
- [x] Implement `Render(*CircuitDef) string` -- generate Mermaid flowchart from parsed definition (P6)
- [x] Create `circuits/rca-investigation.yaml` -- F0-F6 circuit in DSL (P3: reading-first layout)
- [x] Create `circuits/defect-court.yaml` -- D0-D4 circuit in DSL (structure only, no Process implementations)
...
- **Given** the F0-F6 circuit YAML definition,
