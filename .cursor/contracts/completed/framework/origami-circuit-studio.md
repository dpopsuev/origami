# Contract — origami-circuit-studio

**Status:** complete  
**Goal:** Separate web product for circuit visualization, run history, and artifact inspection — like Ansible Tower/AWX for the Origami ecosystem.  
...
- **Separate product.** Circuit Studio is NOT part of the Origami framework. It consumes Origami's structured output (`WalkObserver` events, `Artifact` data, Mermaid diagrams). The framework has no dependency on the Studio.
...
| Feature | Data source |
|---|---|
| Circuit visualization (live Mermaid) | `Render(CircuitDef)`, real-time `WalkEvent` updates |
...
| Multi-circuit dashboard | Multiple `CircuitDef` sources |
...
flowchart TD
    ...
    subgraph studio [Circuit Studio - new product]
        API["Studio API\n(REST/GraphQL)"]
        Store["Event Store\n(Postgres/SQLite)"]
        UI["Web UI\n(React/Svelte)"]
    end
...
|----------|--------|-------------|
| Circuit Studio product spec | `docs/` | domain |
...
- [x] Design UI wireframes (circuit view, run timeline, artifact inspector)
...
- UI wireframes cover circuit visualization, run history, and artifact inspection
