# Contract — origami-k8s-circuit-operator

**Status:** complete  
**Goal:** Circuit YAML runs as a Kubernetes Custom Resource Definition (CRD); nodes execute as Jobs or Pods, edges trigger as events.  
...
- **BYOI capstone.** This is the far end of the infrastructure-agnostic spectrum. The same `CircuitDef` YAML that runs in `origami run` becomes a CRD.
...
- `dsl.go` — `CircuitDef` as the CRD source.
...
- [x] Draft CRD schema (`Circuit`, `CircuitRun` resources)
...
**Then**:
- CRD schema is valid and maps 1:1 to `CircuitDef` YAML
