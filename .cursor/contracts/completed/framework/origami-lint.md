...
- The linter is the static analysis **engine** that the LSP (`origami-lsp`) will embed. Design for embedding: `lint.Run(def *PipelineDef) []Finding` must work without CLI, filesystem, or IO.
...
- E2E scenario YAMLs from `testdata/` and `testdata/scenarios/` are the linter's golden fixtures: valid pipelines must produce zero findings. If a scenario YAML triggers a lint finding, either the rule is wrong or the YAML should be fixed.
...
- **Existing validation:** `PipelineDef.Validate()` (dsl.go:115-174) covers structural basics (non-empty fields, node/edge existence, cross-references). `BuildGraphWith()` catches expression compilation and node/edge resolution errors. `run.Validate()` adds hook existence checks. None of these are rule-based, configurable, severity-aware, or produce machine-readable diagnostics with line numbers.
- **Gaps found in exploration:** No validation for `Element` values, `Merge` strategy values, `Cache.TTL` format, `Imports` resolution, `Input` reference validity, graph reachability, orphaned nodes, WalkerDef element/persona values (validated elsewhere in BuildWalkersFromDef, not in pipeline validation).
- **Agentic workflow impact:** When the AI agent authors or modifies pipeline YAML, `origami lint` runs as a local pipeline stage — catching errors before the first walk. This shifts debugging left: instead of "walk failed because node X doesn't exist," the agent sees "line 14: edge E3 references undefined node 'invesitgate' (did you mean 'investigate'?)". The linter becomes the agent's spell-checker.
...
| **E2E** | yes | `origami lint pipeline.yaml` CLI returns correct exit codes and output |
...
- [ ] **E5** `LintContext`: holds `*PipelineDef`, raw YAML bytes (for line number resolution), file path, `*GraphRegistries` (optional — for deeper checks)
...
- [ ] **E8** `lint.Run(def *PipelineDef, raw []byte, opts ...Option) []Finding` — embeddable entry point (no IO)
...
- [ ] **S8** `missing-pipeline-description` — `description:` is empty. Severity: info.
...
- [ ] **B2** `name-your-edges` — Pipeline has >3 edges without `name:`. Severity: info.
...
- [ ] **B5** `large-pipeline-no-zones` — Pipeline has >6 nodes but no `zones:` defined. Severity: info.
...
- [ ] **F4** `Fix()` implementation for `missing-pipeline-description` — insert `description: ""` placeholder
...
- [ ] **I7** Add `origami lint` as a stage in agent-operations local pipeline (after Build, before Unit test)
...
**Given** a pipeline YAML with `element: fyre` on a node,
**When** `origami lint pipeline.yaml` runs,
...
**Given** a pipeline with a node not reachable from `start:`,
**When** `origami lint pipeline.yaml` runs,
**Then** finding: `warning: G1/orphan-node at line 23: node "unused_step" is not reachable from start node "recall".`

**Given** a pipeline with edges using `condition:` instead of `when:`,
**When** `origami lint --fix pipeline.yaml` runs,
...
**Given** `origami lint --format json pipeline.yaml`,
**When** findings exist,
