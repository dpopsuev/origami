# Contract — Ouroboros Seed Circuit

**Status:** complete  
**Goal:** Replace Ouroboros v1 (custom runner + heuristic keyword scorers) with a standard 3-node Origami circuit (Generator/Subject/Judge) driven by YAML seed definitions with dichotomous pole scoring.  
...
- **Dialectic infrastructure:** `dialectic.go` provides thesis/antithesis/synthesis pattern (`ThesisChallenge`, `AntithesisResponse`, `Synthesis`, `DialecticRound`). The 3-node Ouroboros circuit follows the same structural pattern.
- **Circuit DSL:** `dsl.go` defines `CircuitDef`, `NodeDef`, `EdgeDef`, `ZoneDef`. Standard `graph.Walk()` handles orchestration. The Ouroboros circuit is a standard YAML circuit definition.
- **Conversation context:** Design emerged from a discussion about puzzle-based evaluation where each test is dichotomous (reveals which of two cognitive profiles the AI exhibits), questions are generated fresh each run (no contamination), and the subject doesn't know it's being tested. The "agentic train" insight: Generator→Subject→Judge is a linear circuit, not 3 separate rounds.
...
flowchart LR
    ...
    subgraph circuit [ouroboros-probe.yaml — standard Walk]
        Gen["generate\n(thesis)"]
        Sub["subject\n(antithesis)"]
        Jdg["judge\n(synthesis)"]
    end
...
Phase 1 defines the seed data model. Phase 2 builds the circuit YAML and node extractors. Phase 3 adds types and wires scoring into the existing ModelProfile aggregation. Phase 4 replaces the custom runner with circuit execution. Phase 5 migrates existing probes to seeds and adds new seed categories. Phase 7 adds the Persona Sheet — a per-model routing document combining ModelProfile with circuit step affinity. Phase 8 scopes future calibrate/curate integration (drift detection, seed discrimination, judge agreement). Phase 10 closes the auto-routing loop (PersonaSheet → ProviderRouter via `AutoRouteOption`). Phase 11 validates and tunes.
...
| **Integration** | yes | Full circuit walk with stub dispatcher producing PoleResult |
| **Contract** | yes | Seed YAML schema (must deserialize correctly), PoleResult → ModelProfile interface |
| **E2E** | no | Requires live LLM; stub integration test covers circuit mechanics |
| **Concurrency** | no | Circuit walk is single-goroutine per seed |
...
### Phase 2 — Circuit definition and extractors

- [x] **P1** Create `ouroboros/circuits/ouroboros-probe.yaml` — 3-node linear circuit (generate → subject → judge → DONE)
- [x] **P2** Implement Generator node (`ouroboros/circuit.go`): reads seed, constructs prompt, dispatches to LLM, parses into `GeneratorOutput{Question, PoleAnswers}`
- [x] **P3** Implement Subject node (`ouroboros/circuit.go`): receives only `GeneratorOutput.Question` (no rubric, no poles, no seed metadata). Dispatches prompt. Captures raw response.
- [x] **P4** Implement Judge node (`ouroboros/circuit.go`): receives seed rubric + pole descriptions + subject's raw answer. Produces `PoleResult` via LLM classification.
- [x] **P5** Integration test: walk the circuit with a stub dispatcher, verify Generator→Subject→Judge artifact flow
...
- [x] **R1** Add `origami ouroboros run --seed <path>` CLI command that loads a seed, runs the circuit via `graph.Walk()`, and outputs the `PoleResult`
- [x] **R2** Add `NewSeedProfileConfig` to `ouroborosmcp/` for seed-based circuit dispatch via MCP
...
- [x] **PS2** Implement `EmitPersonaSheet(profile ModelProfile, circuit CircuitDef) (*PersonaSheet, error)` — combines ModelProfile with circuit step affinity
- [x] **PS3** YAML serialization via `MarshalYAML()` — human-readable YAML output
- [x] **PS4** Acceptance criterion verified: all circuit steps with non-zero affinity have persona entries
- [x] **PS5** Unit tests: 7-step RCA circuit + 3-step probe circuit, all steps have persona suggestions
...
- [x] **V1** Validate (green) — `go build ./...` clean, `go test ./...` all pass (except pre-existing flaky dispatch test). Circuit walk produces `PoleResult`. Dimensions aggregate into `ModelProfile`. PersonaSheet emits for all circuit steps.
...
**Then** the circuit walks 3 nodes (generate → subject → judge), the Judge produces a `PoleResult` with a selected pole, confidence, and dimension scores.
...
**Given** a `ModelProfile` and a 7-step circuit definition,  
**When** `EmitPersonaSheet` is called,  
**Then** the resulting `PersonaSheet` contains: model identity, element match, dimension scores, and a suggested persona for each of the 7 circuit steps with non-zero affinity scores.
...
No trust boundaries affected. Seeds are local YAML files. The circuit uses the same dispatcher interface as all other Origami circuits. No new external calls, no new data persistence, no new user input surfaces.
