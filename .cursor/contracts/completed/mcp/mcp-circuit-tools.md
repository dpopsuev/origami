# Contract — MCP Circuit Investigation Tools

**Status:** complete  
**Goal:** MCP tools drive the F0–F6 investigation circuit, with Asterisk as the stateful server holding the heuristic engine and store.  
**Serves:** MCP integration  
**Closure:** De facto implemented. MCP server dispatches circuit steps via `get_next_step`/`submit_artifact` tools. CursorAdapter uses these tools for F0-F6 circuit execution. Heuristic engine and store fully wired.

## Contract rules

- Reuse existing orchestrate, store, metrics, and template packages — do not duplicate logic.
- Each MCP tool maps to a well-defined circuit operation; no tool does more than one concern.
...
- Circuit engine: `internal/orchestrate/` — `BuildParams`, `FillTemplate`, `EvaluateHeuristics`, `ApplyStoreEffects`, `AdvanceStep`, `SaveState`, `InitState`.
...
5. Implement `get_status` tool — returns current circuit state for one or all cases.
...
- Initializes circuit state per case (step = F0_RECALL).
...
- Reads current circuit state from in-memory case states.
...
- [x] **Implement `start_investigation` tool** — Creates session, loads scenario, initializes circuit.
