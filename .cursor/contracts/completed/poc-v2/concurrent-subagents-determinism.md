...
**Status:** complete  
**Goal:** Prove that 4 concurrent Cursor subagents can deterministically drain the full MCP calibration circuit with zero dispatch collisions, no starvation, and a complete final report.  
...
Existing tests (`TestServer_Parallel_*`) exercise `parallel: 2` with at most 2 steps consumed. No test drains the circuit, and no test proves 4 concurrent consumers can operate without starvation or routing errors.

## Desired Architecture

Two new integration tests in `server_test.go`:

1. `TestServer_FourSubagents_FullDrain` -- 4 goroutines drain `ptp-mock` (12 cases) through the full F0-F6 circuit via MCP tool calls. Asserts all 4 got work, all dispatch IDs unique, report complete.
...
- **When** 4 concurrent goroutines drain the circuit,
- **Then** no `(case_id, step)` pair appears twice in the collected work log.

## Notes

- 2026-02-19 — Contract created. Motivated by gap in MCP test coverage: existing tests stop at `parallel: 2` and do not drain the circuit. The `agent-bus.mdc` delegation mandate requires every step to be dispatched to a subagent, but no test proves this works at 4-way concurrency.
