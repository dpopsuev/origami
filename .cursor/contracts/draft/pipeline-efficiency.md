...
**Given** a pipeline with node B having `cache: { ttl: 60s }` and a `WithNodeCache(InMemoryCache)`,  
**When** the pipeline is walked twice with identical input,  
...
**Given** a pipeline with parallel branches merging at node M with `merge: append`,  
**When** branches produce artifacts A1 and A2,  
**Then** node M receives a `ListArtifact` containing `[A1, A2]` as its `PriorArtifact`.

**Given** a pipeline with parallel branches merging at node M with `merge: latest`,  
**When** branches complete in order (A1 first, A2 second),  
**Then** node M receives A2 as its `PriorArtifact`.

**Given** a pipeline with parallel branches merging at node M with no `merge:` field,  
