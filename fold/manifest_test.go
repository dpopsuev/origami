...
`)
	...
	if m.Imports[0] != "origami.marbles.rca" {
...
`)
	...
	if m.CLI.Analyze == nil || m.CLI.Analyze.Provider != "marbles.rca.AnalyzeFunc" {
		t.Error("CLI.Analyze not parsed correctly")
	}
	if m.CLI.Calibrate == nil || m.CLI.Calibrate.Provider != "marbles.rca.CalibrateRunner" {
	...
	if m.Serve == nil || m.Serve.Provider != "marbles.rca.ServeConfig" {
		t.Error("Serve not parsed correctly")
	}
	if m.Demo == nil || m.Demo.Provider != "marbles.rca.DemoConfig" {
