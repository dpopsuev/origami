  ...
  return (
    ...
    >
      <div className="max-w-4xl w-full">
        ...
        {data.description && (
          <p className="text-center text-rh-gray-40 mb-10">{data.description}</p>
        )}
          ...
          {data.metrics.map((m) => (
            ...
            >
              ...
              <div className="w-full bg-rh-gray-60 rounded-full h-3">
                <div
                  className={`${m.color || 'bg-rh-teal-50'} h-3 rounded-full transition-all duration-1000`}
        ...
        {data.summary && data.summary.length > 0 && (
          <div className="mt-8 grid grid-cols-3 gap-4 text-center">
            {data.summary.map((s) => (
              <div key={s.label} className="bg-rh-gray-60/30 rounded-xl p-4">
                <div className={`text-2xl font-bold ${s.color || 'text-rh-teal-30'}`}>
                  {s.value}
                </div>
                <div className="text-xs text-rh-gray-40">{s.label}</div>
              </div>
            ))}
          </div>
        )}
