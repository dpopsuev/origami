...
type DepEdge struct {
	From string // source name that must resolve first
	To   string // source name that depends on From
}

// DepGraph is a directed acyclic graph of source dependencies.
// It produces a topological ordering so that dependent sources are
// resolved after their prerequisites.
...
// Sources not in the graph are appended at the end in their original order.
func (g *DepGraph) OrderSources(sources []Source) ([]Source, error) {
	...
	result := make([]Source, 0, len(ordered))
