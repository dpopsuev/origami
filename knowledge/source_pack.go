...
// SourcePack is a composable collection of knowledge sources for a specific
...
// BranchPattern (or the pack-level VersionKey as fallback).
func (p *SourcePack) ToSources(attrs map[string]string) []Source {
	var sources []Source
	for _, r := range p.Repos {
		pattern := r.BranchPattern
		if pattern == "" && p.VersionKey != "" {
			if v, ok := attrs[p.VersionKey]; ok {
				pattern = "release-" + v
			}
		}
