...
// SessionState tracks the lifecycle of a pipeline session.
...
// PipelineSession holds the state for a single pipeline run driven by MCP
// tool calls. It is domain-agnostic — domain logic lives in RunFunc and
// FormatReport, both provided via PipelineConfig.
...
// NewPipelineSession creates a pipeline session, calls the domain RunFunc
// in a goroutine, and returns immediately. The caller (PipelineServer)
// manages the session lifecycle.
func NewPipelineSession(
	ctx context.Context,
	id string,
	meta SessionMeta,
	parallel int,
	...
	sess := &PipelineSession{
		ID:              id,
		log:             logging.New("pipeline-session"),
		state:           StateRunning,
...
func (s *PipelineSession) GetState() SessionState {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.state
}
...
func (s *PipelineSession) PullerEnter() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.concurrentPullers++
...
func (s *PipelineSession) PullerExit() {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *PipelineSession) AgentPull() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.agentInFlight++
...
func (s *PipelineSession) AgentSubmit() int {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *PipelineSession) SetGateExempt() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.gateExempt = true
}
...
func (s *PipelineSession) CheckCapacityGate() error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *PipelineSession) AgentInFlight() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.agentInFlight
}
...
func (s *PipelineSession) RegisterWorker(id, mode string) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *PipelineSession) WorkerModeStats() (total, stream int) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *PipelineSession) WorkerPrompt(cfg *PipelineConfig) string {
	var sb strings.Builder

	preamble := cfg.WorkerPreamble
	if preamble == "" {
		preamble = fmt.Sprintf("You are a %s pipeline worker.", cfg.Name)
	}
	sb.WriteString(preamble)
	sb.WriteString(" Process pipeline steps by calling MCP tools directly in a loop until the pipeline is drained.\n\n")
...
func (s *PipelineSession) SetTTL(ttl time.Duration) {
	s.mu.Lock()
	s.ttl = ttl
...
func (s *PipelineSession) touchActivity() {
	s.mu.Lock()
...
func (s *PipelineSession) watchdog() {
	s.mu.Lock()
	ttl := s.ttl
	s.mu.Unlock()
...
func (s *PipelineSession) Done() <-chan struct{} {
	return s.doneCh
}

func (s *PipelineSession) run(ctx context.Context, runFn RunFunc) {
	...
	if err != nil {
		...
		s.log.Error("pipeline run failed", "error", err)
		return
	}
	s.state = StateDone
	s.result = result
	s.Bus.Emit("session_done", "server", "", "", map[string]string{})
	s.log.Info("pipeline run complete")
}
...
func (s *PipelineSession) Result() any {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.result
}

// Err returns any error from the pipeline run.
func (s *PipelineSession) Err() error {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.err
}
