...
func AmbiguitySpec() metacal.ProbeSpec {
	return metacal.ProbeSpec{
		...
		Step:        metacal.StepAmbiguity,
		Dimensions: []metacal.Dimension{
			metacal.DimFailureMode,
...
func ScoreAmbiguity(raw string) map[metacal.Dimension]float64 {
	...
	return map[metacal.Dimension]float64{
		metacal.DimFailureMode:          clamp(failureMode),
