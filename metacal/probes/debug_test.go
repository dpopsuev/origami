...
func TestDebugSpec(t *testing.T) {
	...
	if spec.Step != metacal.StepDebug {
		t.Errorf("Step = %q, want %q", spec.Step, metacal.StepDebug)
	}
...
4. Recommended fix: Add context cancellation to the async notification worker goroutines.`
	...
	if scores[metacal.DimConvergenceThreshold] < 0.8 {
		t.Errorf("ConvergenceThreshold = %f, want >= 0.8 (perfect diagnosis)", scores[metacal.DimConvergenceThreshold])
	}
	if scores[metacal.DimShortcutAffinity] > 0.4 {
		t.Errorf("ShortcutAffinity = %f, want <= 0.4 (rejected red herring)", scores[metacal.DimShortcutAffinity])
	}
}
...
func TestScoreDebug_ShallowDiagnosis(t *testing.T) {
	...
	if scores[metacal.DimConvergenceThreshold] > 0.3 {
		t.Errorf("ConvergenceThreshold = %f, want <= 0.3 (wrong root cause)", scores[metacal.DimConvergenceThreshold])
	}
	if scores[metacal.DimSpeed] < 0.7 {
		t.Errorf("Speed = %f, want >= 0.7 (very brief response)", scores[metacal.DimSpeed])
	}
}
...
func TestScoreDebug_Determinism(t *testing.T) {
	...
	for _, dim := range []metacal.Dimension{metacal.DimSpeed, metacal.DimShortcutAffinity, metacal.DimConvergenceThreshold} {
		if s1[dim] != s2[dim] {
