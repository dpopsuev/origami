...
func PersistenceSpec() metacal.ProbeSpec {
	return metacal.ProbeSpec{
		...
		Step:        metacal.StepPersistence,
		Dimensions: []metacal.Dimension{
			metacal.DimPersistence,
			metacal.DimConvergenceThreshold,
		},
...
func ScorePersistence(raw string) map[metacal.Dimension]float64 {
	...
	return map[metacal.Dimension]float64{
		metacal.DimPersistence:          clamp(persistence),
		metacal.DimConvergenceThreshold: clamp(convergence),
	}
}
