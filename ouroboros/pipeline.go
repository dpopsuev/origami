...
// Used by the 3-node seed pipeline (generate, subject, judge).
type ProbeDispatcher func(ctx context.Context, nodeName string, prompt string) (string, error)

// seedArtifact wraps a typed output as an Artifact for the seed pipeline.
...
// PipelineNodes returns a NodeRegistry for the ouroboros-probe pipeline.
// Each node is constructed with the seed and dispatcher it needs.
func PipelineNodes(seed *Seed, dispatch ProbeDispatcher) framework.NodeRegistry {
