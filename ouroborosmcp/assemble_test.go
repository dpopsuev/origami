...
func TestAssembleProfiles_MergesAcrossRuns(t *testing.T) {
	dir := t.TempDir()
	store, err := ouroboros.NewFileRunStore(dir)
	if err != nil {
		...
		Results: []ouroboros.DiscoveryResult{
			{
				...
				Probe: ouroboros.ProbeResult{
					ProbeID:         "refactor-v1",
					DimensionScores: map[ouroboros.Dimension]float64{ouroboros.DimSpeed: 0.8, ouroboros.DimShortcutAffinity: 0.7, ouroboros.DimEvidenceDepth: 0.3},
				},
			},
			{
				Model: claude,
				Probe: ouroboros.ProbeResult{
					ProbeID:         "refactor-v1",
					DimensionScores: map[ouroboros.Dimension]float64{ouroboros.DimSpeed: 0.3, ouroboros.DimShortcutAffinity: 0.2, ouroboros.DimEvidenceDepth: 0.9},
				},
			},
		},
	...
	run2 := ouroboros.RunReport{
		RunID:     "run-debug",
		StartTime: time.Now(),
		EndTime:   time.Now(),
		Config:    ouroboros.DiscoveryConfig{ProbeID: "debug-v1"},
		Results: []ouroboros.DiscoveryResult{
			{
				Model: gpt4o,
				Probe: ouroboros.ProbeResult{
					ProbeID:         "debug-v1",
					DimensionScores: map[ouroboros.Dimension]float64{ouroboros.DimSpeed: 0.9, ouroboros.DimConvergenceThreshold: 0.6},
				},
			},
			{
				Model: claude,
				Probe: ouroboros.ProbeResult{
					ProbeID:         "debug-v1",
					DimensionScores: map[ouroboros.Dimension]float64{ouroboros.DimSpeed: 0.4, ouroboros.DimConvergenceThreshold: 0.9},
				},
			},
		},
		UniqueModels: []framework.ModelIdentity{gpt4o, claude},
	...
	for _, p := range out.Profiles {
		key := ouroboros.ModelKey(p.Model)
		...
		switch key {
		case "gpt-4o":
			speed := p.Dimensions[ouroboros.DimSpeed]
		...
		case "claude-sonnet-4":
			...
			conv := p.Dimensions[ouroboros.DimConvergenceThreshold]
...
func TestAssembleProfiles_SkipsResultsWithoutDimensionScores(t *testing.T) {
	dir := t.TempDir()
	store, _ := ouroboros.NewFileRunStore(dir)
		...
		Results: []ouroboros.DiscoveryResult{
			{
				...
				Probe: ouroboros.ProbeResult{
					ProbeID: "refactor-v1",
					Score:   ouroboros.ProbeScore{TotalScore: 0.5},
				},
			},
		},
