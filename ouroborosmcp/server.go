...
// NewServer creates an Ouroboros MCP server with discovery tools registered.
func NewServer(runsDir string) *Server {
	fw := fwmcp.NewServer("ouroboros", "dev")
	s := &Server{
		MCPServer: fw.MCPServer,
		RunsDir:   runsDir,
		Probes:    NewProbeRegistry(),
		log:       slog.Default().With("component", "ouroboros-mcp"),
	}
	s.registerTools()
	return s
}
...
type submitDiscoveryResponseOutput struct {
	...
	DimensionScores map[ouroboros.Dimension]float64 `json:"dimension_scores,omitempty"`
...
type getDiscoveryReportOutput struct {
	...
	Report       *ouroboros.RunReport        `json:"report,omitempty"`
...
func (s *Server) handleGetDiscoveryReport(_ context.Context, _ *sdkmcp.CallToolRequest, input getDiscoveryReportInput) (*sdkmcp.CallToolResult, getDiscoveryReportOutput, error) {
	...
	if s.RunsDir != "" && !alreadyPersisted {
		store, storeErr := ouroboros.NewFileRunStore(s.RunsDir)
		if storeErr == nil {
...
func (s *Server) assembleProfilesFromStore() (assembleProfilesOutput, error) {
	...
	store, err := ouroboros.NewFileRunStore(s.RunsDir)
	if err != nil {
