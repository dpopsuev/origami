...
func TestSession_NewSession_InitialState(t *testing.T) {
	s := NewSession(ouroboros.DefaultConfig(), nil)
...
func TestSession_NewSession_ZeroConfig_UsesDefaults(t *testing.T) {
	s := NewSession(ouroboros.DiscoveryConfig{}, nil)
...
func TestSession_SubmitResponse_AfterMaxIterations_Done(t *testing.T) {
	s := NewSession(ouroboros.DiscoveryConfig{
		MaxIterations:     2,
		ProbeID:           "refactor-v1",
...
func TestSession_ConcurrentFinalize_NoRace(t *testing.T) {
	...
	var wg sync.WaitGroup
	reports := make([]*ouroboros.RunReport, 10)
	for i := 0; i < 10; i++ {
		wg.Add(1)
		go func(idx int) {
			defer wg.Done()
...
func TestSession_DebugProbe_ProducesDimensionScores(t *testing.T) {
	handler := &ProbeHandler{
		...
		Score:          func(raw string) map[ouroboros.Dimension]float64 { return map[ouroboros.Dimension]float64{ouroboros.DimSpeed: 0.5, ouroboros.DimConvergenceThreshold: 0.8} },
	...
	if result.Probe.DimensionScores[ouroboros.DimSpeed] != 0.5 {
		t.Errorf("DimSpeed = %v, want 0.5", result.Probe.DimensionScores[ouroboros.DimSpeed])
	}
	if result.Probe.DimensionScores[ouroboros.DimConvergenceThreshold] != 0.8 {
		t.Errorf("DimConvergenceThreshold = %v, want 0.8", result.Probe.DimensionScores[ouroboros.DimConvergenceThreshold])
	}
}
...
func TestSession_DebugProbe_NextPrompt_UsesProbeHandler(t *testing.T) {
	...
	handler := &ProbeHandler{
		...
		Score:          func(raw string) map[ouroboros.Dimension]float64 { return nil },
		NeedsCodeBlock: false,
	}
	s := NewSession(ouroboros.DiscoveryConfig{
		MaxIterations: 10,
...
func TestSession_DebugProbe_NoCodeBlock_NoError(t *testing.T) {
	handler := &ProbeHandler{
		...
		Score:          func(raw string) map[ouroboros.Dimension]float64 { return map[ouroboros.Dimension]float64{ouroboros.DimSpeed: 0.5} },
		NeedsCodeBlock: false,
	}
	s := NewSession(ouroboros.DiscoveryConfig{
		MaxIterations: 10,
