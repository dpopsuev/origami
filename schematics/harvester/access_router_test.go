...
func TestRouter_DispatchByKind(t *testing.T) {
	...
	router := harvester.NewRouter(
		harvester.WithGitDriver(gitDriver),
		harvester.WithDocsDriver(docDriver),
	)
...
func TestRouter_Ensure(t *testing.T) {
	...
	router := harvester.NewRouter(harvester.WithGitDriver(driver))
...
func TestRouter_ReaderInterface(t *testing.T) {
	var r toolkit.SourceReader = harvester.NewRouter()
	if r == nil {
...
func TestRouter_WithOfflineFS(t *testing.T) {
	...
	router := harvester.NewRouter(harvester.WithOfflineFS(bundle))
