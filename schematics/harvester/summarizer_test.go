...
func TestTruncateSummarizer_Full_FitsInBudget(t *testing.T) {
	s := harvester.TruncateSummarizer{}
	content := "short content"
	got := s.Summarize(content, 100, harvester.StrategyFull)
	if got != content {
...
func TestTruncateSummarizer_Full_Truncated(t *testing.T) {
	...
	got := s.Summarize(content, 50, harvester.StrategyFull)
...
func TestTruncateSummarizer_Summary(t *testing.T) {
	...
	got := s.Summarize(content, 50, harvester.StrategySummary)
...
func TestTruncateSummarizer_OnDemand(t *testing.T) {
	s := harvester.TruncateSummarizer{}
	got := s.Summarize("hello world", 10, harvester.StrategyOnDemand)
...
func TestTruncateSummarizer_IndexOnly(t *testing.T) {
	s := harvester.TruncateSummarizer{}
	content := "line1\nline2\nline3"
	got := s.Summarize(content, 10, harvester.StrategyIndexOnly)
...
func TestBudgetAllocator_Allocate(t *testing.T) {
	ba := harvester.BudgetAllocator{TotalBudget: 1000}
	...
	if entries[0].Strategy != harvester.StrategyFull {
		t.Errorf("always-read should get StrategyFull, got %s", entries[0].Strategy)
	}
	if entries[1].Strategy != harvester.StrategySummary {
