...
func (t *treeTransformer) Name() string { return "harvester.tree" }
...
func (t *searchTransformer) Name() string { return "harvester.search" }
...
func (t *readTransformer) Name() string { return "harvester.read" }
...
// It looks for a dedicated "harvester.search_keywords" key first (set by
...
func extractSearchKeywords(ws *framework.WalkerState) []string {
	...
	if kw, ok := ws.Context["harvester.search_keywords"].([]string); ok && len(kw) > 0 {
		return kw
	}
