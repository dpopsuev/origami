...
func TestKnowledgeCircuit_Walk(t *testing.T) {
	reader := &txReader{
	...
	def, err := framework.LoadCircuit([]byte(knowledgeCircuitYAML))
	if err != nil {
	...
	walker := framework.NewProcessWalker("test-knowledge")
	walker.State().Context["knowledge.search_keywords"] = []string{"TestPTP"}
...
func TestKnowledgeCircuit_EmptyCatalog(t *testing.T) {
	reader := &txReader{}
