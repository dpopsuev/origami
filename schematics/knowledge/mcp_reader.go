...
func (r *MCPReader) Ensure(ctx context.Context, src Source) error {
...
func (r *MCPReader) Search(ctx context.Context, src Source, query string, maxResults int) ([]SearchResult, error) {
	args := map[string]any{
		"source":      src,
		"query":       query,
		"max_results": maxResults,
	}
	results, err := subprocess.CallToolTyped[[]SearchResult](ctx, r.caller, "knowledge_search", args)
	if err != nil {
...
func (r *MCPReader) Read(ctx context.Context, src Source, path string) ([]byte, error) {
	args := map[string]any{
		"source": src,
		"path":   path,
	}
...
func (r *MCPReader) List(ctx context.Context, src Source, root string, maxDepth int) ([]ContentEntry, error) {
	args := map[string]any{
		"source":    src,
		"root":      root,
		"max_depth": maxDepth,
	}
	entries, err := subprocess.CallToolTyped[[]ContentEntry](ctx, r.caller, "knowledge_list", args)
	if err != nil {
