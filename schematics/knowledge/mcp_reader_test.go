...
func TestMCPReader_Ensure(t *testing.T) {
	...
	if len(mock.calls) != 1 || mock.calls[0].Name != "knowledge_ensure" {
		t.Fatalf("expected 1 call to knowledge_ensure, got %v", mock.calls)
	}
}
...
func TestMCPReader_Search(t *testing.T) {
	...
	if mock.calls[0].Name != "knowledge_search" {
		t.Errorf("tool name = %q, want knowledge_search", mock.calls[0].Name)
	}
}
...
func TestMCPReader_EnsureErrorResult(t *testing.T) {
	...
	reader := skn.NewMCPReader(errMock)
