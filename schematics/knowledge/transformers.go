...
// CodeContext is the aggregate output of the Knowledge circuit.
...
// TransformerComponent returns a Component containing the Knowledge circuit
...
func (t *treeTransformer) Name() string { return "knowledge.tree" }
...
func (t *searchTransformer) Name() string { return "knowledge.search" }
...
func (t *readTransformer) Name() string { return "knowledge.read" }
...
// It looks for a dedicated "knowledge.search_keywords" key first (set by
...
func extractSearchKeywords(ws *framework.WalkerState) []string {
	...
	if kw, ok := ws.Context["knowledge.search_keywords"].([]string); ok && len(kw) > 0 {
		return kw
	}
