...
function pipelineToFlow(def: PipelineDef): { nodes: Node[]; edges: Edge[] } {
...
const DEMO_PIPELINE: PipelineDef = {
  pipeline: "demo",
  nodes: [
...
export function PipelineGraph() {
  const [pipeline, setPipeline] = useState<PipelineDef>(DEMO_PIPELINE);

  const { nodes: initialNodes, edges: initialEdges } = useMemo(
    () => pipelineToFlow(pipeline),
    [pipeline]
  );
  ...
  useEffect(() => {
    const { nodes: n, edges: e } = pipelineToFlow(pipeline);
    setNodes(n);
    setEdges(e);
  ...
  useEffect(() => {
    const handler = async () => {
      try {
        ...
        // API not available, keep demo pipeline
      }
    };
    handler();
  }, []);
